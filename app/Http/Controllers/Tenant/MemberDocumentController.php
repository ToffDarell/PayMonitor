...
class MemberDocumentController extends Controller
{
    ...
    public function store(Request $request, string $tenant, Member $member): RedirectResponse
    {
        abort_unless(
            $request->user()?->hasTenantPermission(TenantPermissions::MEMBER_DOCUMENTS_UPLOAD, ['tenant_admin', 'branch_manager', 'loan_officer']),
            403,
    ...
    public function destroy(Request $request, string $tenant, MemberDocument $document): RedirectResponse
    {
        abort_unless(
            $request->user()?->hasTenantPermission(TenantPermissions::MEMBER_DOCUMENTS_DELETE, ['tenant_admin']),
            403,
            'This action is unauthorized.'
        );

        $document->loadMissing('member');
        $this->authorize('view', $document->member);
    ...
    public function download(Request $request, string $tenant, MemberDocument $document)
    {
        abort_unless(
            $request->user()?->hasTenantPermission(TenantPermissions::MEMBER_DOCUMENTS_VIEW, ['tenant_admin', 'branch_manager', 'loan_officer', 'viewer']),
            403,
            'This action is unauthorized.'
        );
