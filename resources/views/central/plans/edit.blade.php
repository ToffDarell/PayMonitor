...
@section('content')
<div class="mb-8">
    <h2 class="font-heading text-2xl font-bold tracking-tight text-white">Edit Plan</h2>
    <p class="mt-1 text-sm text-slate-400">Update limits and pricing for {{ $plan->name }}.</p>
</div>

<div class="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-6 sm:p-8">
    <form method="POST" action="{{ route('central.plans.update', $plan, false) }}">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div>
                ...
                <input type="number" id="max_branches" name="max_branches" value="{{ old('max_branches', $plan->max_branches) }}" required
            ...
            <div>
                ...
                <input type="number" id="max_users" name="max_users" value="{{ old('max_users', $plan->max_users) }}" required
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="description" name="description" rows="4"
                    class="block w-full rounded-xl border @error('description') border-red-500/50 @else border-white/10 @enderror bg-white/[0.03] px-4 py-3 text-sm text-slate-100 placeholder-slate-500 transition focus:border-emerald-500/60 focus:outline-none focus:ring-2 focus:ring-emerald-500/20">{{ old('description', filled($plan->description) ? $plan->description : \App\Models\Plan::defaultDescription()) }}</textarea>
