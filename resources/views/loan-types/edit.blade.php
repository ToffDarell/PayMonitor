...
<div class="card border-0 shadow-sm">
    <div class="card-body p-4" x-data="loanTypePresetForm({ showPresets: false })">
        <form action="{{ route('loan-types.update', [...$tenantParameter, 'loan_type' => $loanType]) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="d-flex justify-content-end mb-4">
                <button type="button" class="btn btn-outline-success" x-on:click="showPresets = !showPresets" x-bind:aria-expanded="showPresets.toString()">
                    <i class="bi me-2" :class="showPresets ? 'bi-chevron-up' : 'bi-chevron-down'"></i>Load from preset
                </button>
            </div>

            <div x-cloak x-show="showPresets" x-transition.opacity.duration.200ms class="loan-type-preset-panel p-4 p-lg-5 mb-4">
                <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3 mb-4">
                    <div>
                        <h2 class="h4 fw-bold text-white mb-2">Quick Select a Loan Type (Optional)</h2>
                        <p class="text-secondary-emphasis mb-0">Choose a preset to auto-fill the form, then fine-tune the values before saving.</p>
                    </div>
                ...
                <div class="loan-type-divider mt-4">Or customize the loan type below</div>
            </div>
            ...
            <div class="row g-4">
                <div class="col-lg-8">
                    <div class="mb-3">
                        ...
                        <input type="text" id="name" name="name" x-ref="name" value="{{ old('name', $loanType->name) }}" class="form-control @error('name') is-invalid @enderror" required>
                        @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label fw-semibold">Description</label>
                        <textarea id="description" name="description" x-ref="description" rows="4" class="form-control @error('description') is-invalid @enderror">{{ old('description', $loanType->description) }}</textarea>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" min="0" id="interest_rate" name="interest_rate" x-ref="interestRate" value="{{ old('interest_rate', $loanType->interest_rate) }}" class="form-control @error('interest_rate') is-invalid @enderror" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" min="1" id="max_term_months" name="max_term_months" x-ref="maxTermMonths" value="{{ old('max_term_months', $loanType->max_term_months) }}" class="form-control @error('max_term_months') is-invalid @enderror">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group" x-data="{ raw: '{{ old('max_amount', $loanType->max_amount) }}', format(n) { if (!n) return ''; let num = n.replace(/,/g, ''); if (!num || isNaN(num)) return n; let parts = num.split('.'); parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ','); return parts.join('.'); } }">
                                <span class="input-group-text">P</span>
                                <input type="text" inputmode="decimal" id="max_amount" x-ref="input" x-on:input="raw = $event.target.value.replace(/[^0-9.]/g, ''); $refs.input.value = format(raw)" x-on:keydown="if ($event.ctrlKey || $event.metaKey) return; if (!'0123456789.'.includes($event.key) && $event.key.length === 1) $event.preventDefault()" class="form-control @error('max_amount') is-invalid @enderror" x-init="$el.value = format(raw)" data-format-currency>
                ...
                <div class="col-lg-4">
                    <div class="border rounded-3 p-4 bg-light h-100">
                        ...
                        <div class="form-check border rounded-3 bg-white p-3 mb-3">
                            <input class="form-check-input" type="radio" name="interest_type" id="interest_type_flat" x-ref="interestTypeFlat" value="flat" @checked(old('interest_type', $loanType->interest_type) === 'flat')>
                        ...
                        <div class="form-check border rounded-3 bg-white p-3 mb-3">
                            <input class="form-check-input" type="radio" name="interest_type" id="interest_type_diminishing" x-ref="interestTypeDiminishing" value="diminishing" @checked(old('interest_type', $loanType->interest_type) === 'diminishing')>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('loanTypePresetForm', ({ showPresets = false } = {}) => ({
            selectedPreset: '',
            showPresets,
