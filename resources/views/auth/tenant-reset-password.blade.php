...
<head>
    ...
    <title>Reset Password | {{ $tenantName }} Portal</title>
            ...
            <div class="relative flex min-h-screen w-full flex-col justify-between px-10 py-10 lg:px-14">
                ...
                <div class="mx-auto max-w-md">
                    ...
                    <p class="mt-4 max-w-sm text-base leading-7 text-slate-400">
                        Choose a new password for your tenant dashboard access.
                    </p>
                </div>
        ...
        <main class="flex min-h-screen items-center justify-center bg-[#0B1120] px-5 py-10 sm:px-8">
            <div class="w-full max-w-md rounded-2xl border border-white/[0.08] bg-white/[0.03] p-7 shadow-[0_24px_80px_rgba(0,0,0,0.5)] backdrop-blur sm:p-8" x-data="{ showPassword: false, showConfirm: false, password: '', confirmPassword: '' }">
                <div>
                    ...
                    <p class="mt-2 text-sm leading-6 text-slate-400">Set a new password for your tenant user account.</p>
                </div>

                <form method="POST" action="{{ route('tenant.password.store', ['tenant' => request()->route('tenant')], false) }}" class="mt-8 space-y-5">
                    @csrf
                    ...
                    <div>
                        ...
                        <input id="email" name="email" type="email" value="{{ old('email', $request->email) }}" autocomplete="username" required autofocus class="pm-input block w-full rounded-xl px-4 py-3 text-sm">
                        @error('email')
